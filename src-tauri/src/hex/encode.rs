...
pub fn hex_encode_custom<'a>(
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        match super::vectorization_support() {
            super::Vectorization::AVX2 => unsafe { hex_encode_avx2(src, dst, upper_case) },
            super::Vectorization::SSE41 => unsafe { hex_encode_sse41(src, dst, upper_case) },
            super::Vectorization::None => hex_encode_custom_case_fallback(src, dst, upper_case),
        }
    ...
    {
        match super::vectorization_support() {
            super::Vectorization::Neon => unsafe { hex_encode_neon(src, dst, upper_case) },
            super::Vectorization::None => hex_encode_custom_case_fallback(src, dst, upper_case),
        }
        // Safety: We just wrote valid utf8 hex string into the dst
        return Ok(unsafe { mut_str(dst) });
